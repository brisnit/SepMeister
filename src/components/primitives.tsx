...
import { placeTooltip, type TooltipPlacement } from "@/lib/ui/tooltip";
 ...
 * A portal escapes the clip entirely; `placeTooltip` then keeps it on screen.
 */
export function InfoTip({ text, children }: { text: string; children?: ReactNode }) {
  ...
  const place = useCallback(() => {
    ...
    // Measure the rendered tooltip when it exists, so the above/below decision
    // uses its real height rather than an assumed one.
    const height = tipRef.current?.getBoundingClientRect().height ?? 0;
    setPlacement(
      placeTooltip(
  ...
  // Re-measure once the tooltip has rendered, and keep it with its trigger
  // while the panel behind it scrolls.
  useEffect(() => {
    if (!open) return;
    place();
