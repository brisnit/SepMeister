...
const CSV_COLUMNS: [string, (r: FeedbackRecord) => unknown][] = [
  ...
  ["notes", (r) => r.notes],
  ["printer_make", (r) => r.outputSetup?.printerManufacturer ?? ""],
  ["printer_model", (r) => r.outputSetup?.printerModel ?? ""],
  ["rip", (r) => r.outputSetup?.rip ?? ""],
  ["output_resolution", (r) => r.outputSetup?.outputResolution ?? ""],
  ["media", (r) => r.outputSetup?.media ?? ""],
  ["density_settings", (r) => r.outputSetup?.densitySettings ?? ""],
  ["other_rip_settings", (r) => r.outputSetup?.otherRipSettings ?? ""],
